...
turtles-own [
  ...
  standing                                   ; this agent's current standing (is true (i.e., 'good') when agent cooperates, false (i.e., 'bad') in case of unjustified defection)
...
to setup
  ...
  ; Initialize agents (initial balances are defined as input in the main visualization screen):
  ask turtles [
    set fitness 0                            ; agents are initialized with zero fitness
    set memory []                            ; agents are initialized with empty memory lists
    set standing true                        ; agents are initialized with 'good' standing
...
to go
  ...
  ask turtles [
    ...
    ;; d. Indirect-reciprocity agents cooperate only if their current partner's standing is 'good' (true):
    if breed = indirects [
      ifelse [standing] of current-partner [
        cooperate
      ][
        defect
      ]
...
to cooperate
  ...
  set standing true                                                  ; this agent's standing is 'good' (true)
...
to defect
  ...
  if [standing] of current-partner [                                 ; unjustified defection. It happens when defecting with a partner with 'good' standing. It sets defecting agent's standing to 'bad' (false)
    set standing false
  ]
...
265
553
Indirect Reciprocators:\nonly help if partner's standing is 'good'
10
2.0
1
...
4. indirect-reciprocators, who help their current partners as long as the partner's standing is 'good'
...
For money, further parameters can be configured:
- the initial money balances across agents ("liquidity")
Agents using the money strategy make their decisions to cooperate based on whether money balances of partners are positive.


## THINGS TO NOTICE

The monetary exchange strategy is selected for a wide variety of benefit-to-cost ratios. 
